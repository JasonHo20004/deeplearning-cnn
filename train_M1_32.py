...
# -*- coding: utf-8 -*-
"""
Train M1 model on 32x32 animal dataset (chicken, cow, monkey).
Uses pre-resized 32x32 images from datasets/train/32x32/.
Logs training/validation accuracy and loss per epoch to CSV and TXT.

Usage:
    python train_M1_32.py
"""
...
TRAIN_DIR = os.path.join(DATASETS_DIR, 'train', '32x32')
VAL_DIR = os.path.join(DATASETS_DIR, 'validate')
CHECKPOINT_DIR = './checkpoints/M1_32'
LOG_CSV = os.path.join(CHECKPOINT_DIR, 'training_log_32.csv')
LOG_TXT = os.path.join(CHECKPOINT_DIR, 'training_log_32.txt')
...
def prepare_cow_validation():
    ...
    # Fallback: copy from train/32x32/cow
    train_cow = os.path.join(DATASETS_DIR, 'train', '32x32', 'cow')
    if os.path.exists(train_cow):
        ...
        print(f"  -> Copied {n_pick} images from train/32x32/cow (WARNING: slight train/val overlap)")
    else:
...
def main():
    print("=" * 70)
    print("  M1 Training — 32x32 — chicken / cow / monkey")
    print("=" * 70)
    ...
    # ---- Data transforms ----
    # Training: images are already 32x32, apply standard CIFAR-style augmentation
    ...
    # Validation: original-resolution images -> resize to 32x32
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if is_best:
            ...
            }, os.path.join(CHECKPOINT_DIR, 'best_model_32.pth'))
    ...
    print(f'  Best model: {os.path.join(CHECKPOINT_DIR, "best_model_32.pth")}')
    print('=' * 70)
